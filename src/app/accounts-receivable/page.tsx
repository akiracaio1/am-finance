    ...
    try {
      ...
      rows.forEach((row, index) => {
        ...
        if (!vencimentoRaw || !cliente || !categoriaNome || !valor) {
          errors.push(`Linha ${line}: Campos obrigatórios faltando (Vencimento, Cliente, Categoria ou Valor).`);
          return;
        }

        const category = leafCategories.find(c => c.name.toLowerCase().trim() === String(categoriaNome).toLowerCase().trim());
        if (!category) {
          errors.push(`Linha ${line}: Categoria '${categoriaNome}' não cadastrada no plano de contas.`);
          return;
        }
        ...
        validData.push({
          ...
          description: String(row["Descricao"] || "Receita Importada via Excel"),
          amount: Number(valor),
      ...
      if (errors.length > 0) {
        toast({ 
          ...
          description: `Importação cancelada: ${errors.slice(0, 2).join(" | ")}` 
        });
      } else {
        validData.forEach(d => {
          setDocumentNonBlocking(doc(db, "users", user.uid, "accountsReceivableEntries", d.id), d, { merge: true });
        });
        toast({ title: "Importação Concluída", description: `${validData.length} recebimentos foram importados.` });
      }
    } catch (err: any) {
    ...
    <div className="space-y-6 pb-10">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground">Monitoramento inteligente de faturamento e entradas.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-muted/50"><CardHeader className="p-4 pb-2 text-[10px] font-bold text-muted-foreground uppercase">Projeção Total</CardHeader><CardContent className="p-4 pt-0 text-2xl font-bold">R$ {(totalOpen + totalPaid).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</CardContent></Card>
      </div>

      <Card>
        <CardContent className="pt-6">
          <Table>
            <TableHeader><TableRow><TableHead>Previsão</TableHead><TableHead>Origem</TableHead><TableHead>Descrição</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Valor</TableHead><TableHead className="w-[50px]"></TableHead></TableRow></TableHeader>
            <TableBody>
              {entries?.sort((a,b) => a.dueDate.localeCompare(b.dueDate)).map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>{entry.status === 'Paid' ? <Badge className="bg-emerald-100 text-emerald-700">Recebido</Badge> : <Badge variant="outline">Aguardando</Badge>}</TableCell>
      ...
      <Dialog open={isNewEntryOpen} onOpenChange={setIsNewEntryOpen}>
        <DialogContent>
          <form onSubmit={handleSaveEntry}>
            ...
            <DialogFooter><Button type="submit" className="bg-accent text-accent-foreground w-full">Salvar</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
    </div>
